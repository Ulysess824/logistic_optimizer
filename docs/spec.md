...
El sistema es **SOTA (State of the Art)** en logística de backhauling. Utiliza **Google Routes API v2** con perfiles de camión pesados y una caché persistente en SQLite. La lógica de optimización se basa en **Disjunctions (Soft Constraints)** acopladas elásticamente a **Restricciones Duras Volumétricas (Pallets)** mediante `ActiveVar`, garantizando siempre una solución válida sin overflow o cuelgues matemáticos al rechazar nodos excedentes. Soporta optimización multi-planta (`VARIAS_PLANTAS`) y posee una herramienta de visualización geométrica interactiva.
